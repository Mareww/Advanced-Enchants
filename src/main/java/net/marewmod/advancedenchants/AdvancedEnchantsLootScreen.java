...
public class AdvancedEnchantsLootScreen extends Screen {
    ...
    public AdvancedEnchantsLootScreen(Screen parent) {
        ...
        for (Map.Entry<String, AdvancedEnchantsConfig.EnchantmentConfig> entry : cfg.enchantments.entrySet()) {
            ...
            Enchantment ench = Registries.ENCHANTMENT.get(
                new Identifier(AdvancedEnchantsMod.MOD_ID, entry.getKey()));
    ...
    public void close() {
        ...
        for (int p = 0; p < enchIds.size(); p++) {
            AdvancedEnchantsConfig.EnchantmentConfig enchCfg = cfg.enchantments.get(enchIds.get(p));
            if (enchCfg == null) continue;
            List<AdvancedEnchantsConfig.LootEntry> newLoot = new ArrayList<>();
            for (LootRow row : pages.get(p)) {
                if (row.table.isBlank()) continue;
                AdvancedEnchantsConfig.LootEntry le = new AdvancedEnchantsConfig.LootEntry();
                le.table = row.table;
