...
public class AdvancedEnchantmentsConfigScreen extends Screen {
    ...
    public AdvancedEnchantmentsConfigScreen(Screen parent) {
        super(Text.literal("Advanced Enchantments Config"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        for (Map.Entry<String, AdvancedEnchantmentsConfig.EnchantmentConfig> entry : cfg.enchantments.entrySet()) {
            ...
            AdvancedEnchantmentsConfig.EnchantmentConfig enchCfg = entry.getValue();
            Enchantment ench = Registries.ENCHANTMENT.get(new Identifier(AdvancedEnchantmentsMod.MOD_ID, id));
            ...
            if (enchCfg.table_enchantable) {
                Enchantment enchObj = Registries.ENCHANTMENT.get(new Identifier(AdvancedEnchantmentsMod.MOD_ID, id));
        ...
        addDrawableChild(ButtonWidget.builder(Text.literal("Edit Loot Tables"), btn -> {
            ...
            client.setScreen(new AdvancedEnchantmentsLootScreen(this));
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < iconEnchIds.size(); i++) {
            AdvancedEnchantmentsConfig.EnchantmentConfig ec = cfg.enchantments.get(iconEnchIds.get(i));
    ...
    public void close() {
        ...
        if (server != null) {
            server.execute(() -> {
                for (var player : server.getPlayerManager().getPlayerList()) {
                    ...
                    for (int i = 0; i < player.getInventory().size(); i++) {
                        if (AdvancedEnchantmentsConfig.stripDisabled(player.getInventory().getStack(i)))
                            stripped = true;
                    }
                    if (stripped) {
                        ...
                        player.sendMessage(Text.literal(
                            "[Advanced Enchantments] Some disabled enchantments were removed from your items."));
                    }
                }
            ...
            client.player.sendMessage(Text.literal(
                "[Advanced Enchantments] Config saved. Ask the server admin to run /reload."));
        }
