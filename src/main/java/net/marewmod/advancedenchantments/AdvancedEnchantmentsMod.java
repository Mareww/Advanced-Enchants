...
import net.marewmod.advancedenchantments.enchantment.DoubleShotEnchantment;
import net.marewmod.advancedenchantments.enchantment.EvasionEnchantment;
import net.marewmod.advancedenchantments.enchantment.HomingEnchantment;
import net.marewmod.advancedenchantments.enchantment.ExperienceEnchantment;
import net.marewmod.advancedenchantments.enchantment.FlameWalkerEnchantment;
import net.marewmod.advancedenchantments.enchantment.LifestealEnchantment;
import net.marewmod.advancedenchantments.enchantment.MagneticEnchantment;
import net.marewmod.advancedenchantments.enchantment.ReplantEnchantment;
import net.marewmod.advancedenchantments.enchantment.AutoSmeltEnchantment;
import net.marewmod.advancedenchantments.enchantment.TreecapitatorEnchantment;
import net.marewmod.advancedenchantments.enchantment.TripleShotEnchantment;
import net.marewmod.advancedenchantments.enchantment.VeinMinerEnchantment;
...
public class AdvancedEnchantmentsMod implements ModInitializer {
    ...
    public static final String MOD_ID = "advancedenchantments";
    ...
    public void onInitialize() {
        AdvancedEnchantmentsConfig.load();
        ...
        ServerLifecycleEvents.END_DATA_PACK_RELOAD.register((server, resourceManager, success) -> {
            ...
            for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                ...
                for (int i = 0; i < player.getInventory().size(); i++) {
                    if (AdvancedEnchantmentsConfig.stripDisabled(player.getInventory().getStack(i))) stripped = true;
                }
                if (stripped) {
                    player.getInventory().markDirty();
                    player.sendMessage(Text.literal(
                        "[Advanced Enchantments] One or more disabled enchantments were removed from your items."));
                }
            }
        });
        ...
        LootTableEvents.MODIFY.register((rm, manager, id, tableBuilder, source) -> {
            ...
            for (Map.Entry<String, AdvancedEnchantmentsConfig.EnchantmentConfig> entry
                    : AdvancedEnchantmentsConfig.get().enchantments.entrySet()) {
                ...
                for (AdvancedEnchantmentsConfig.LootEntry le : entry.getValue().loot) {
                    if (!AdvancedEnchantmentsConfig.matchesTable(le.table, tableStr)) continue;
        ...
        for (Map.Entry<String, AdvancedEnchantmentsConfig.EnchantmentConfig> enchEntry
                : AdvancedEnchantmentsConfig.get().enchantments.entrySet()) {
            if (!enchEntry.getValue().enabled) continue;
            String enchId = enchEntry.getKey();
            for (AdvancedEnchantmentsConfig.TradeEntry te : enchEntry.getValue().trades) {
                if ("villager".equals(te.type)) {
                    VillagerProfession prof = Registries.VILLAGER_PROFESSION
                        .get(Identifier.tryParse(te.profession));
                    if (prof == null) {
                        LOGGER.warn("[AdvancedEnchantments] Unknown profession '{}' for '{}' trade – skipping",
                    ...
                    TradeOfferHelper.registerVillagerOffers(prof, capturedVillagerLevel, factories ->
                        factories.add((entity, random) -> {
                            if (!AdvancedEnchantmentsConfig.isEnabled(enchId)) return null;
                            if (!AdvancedEnchantmentsConfig.isTradesEnabled(enchId)) return null;
        ...
        ServerTickEvents.END_SERVER_TICK.register(server -> {
            for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                ...
                if (!AdvancedEnchantmentsConfig.isEnabled("flame_walker")) continue;
        ...
        PlayerBlockBreakEvents.AFTER.register((world, player, pos, state, blockEntity) -> {
            ...
            if (!AdvancedEnchantmentsConfig.isEnabled("replant")) return;
        ...
        PlayerBlockBreakEvents.AFTER.register((world, player, pos, state, blockEntity) -> {
            ...
            if (!AdvancedEnchantmentsConfig.isEnabled("treecapitator")) return;
        ...
        PlayerBlockBreakEvents.AFTER.register((world, player, pos, state, blockEntity) -> {
            if (!(world instanceof ServerWorld sw)) return;
            if (CHOPPING.get()) return;
            if (!AdvancedEnchantmentsConfig.isEnabled("vein_miner")) return;
        ...
        ServerLivingEntityEvents.ALLOW_DAMAGE.register((entity, source, amount) -> {
            ...
            if (!AdvancedEnchantmentsConfig.isEnabled("evasion")) return true;
            int pieces = 0;
        ...
        Registry.register(Registries.ITEM_GROUP, new Identifier(MOD_ID, "enchantment_books"),
            FabricItemGroup.builder()
                .displayName(Text.literal("Advanced Enchantments"))
                .icon(() -> {
        ...
        LOGGER.info("Advanced Enchantments loaded.");
    }
